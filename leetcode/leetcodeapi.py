...
def return_contests(link, i):
    ...
    response = requests.post('https://leetcode.com/graphql/', cookies=cookies, headers=headers, json=json_data)
    data = json.loads(response.text)
    total = data['data']['userContestRankingHistory'][-i]['totalProblems']
    title = data['data']['userContestRankingHistory'][-i]['contest']['title']
    time = data['data']['userContestRankingHistory'][-i]['contest']['startTime']
    return total, title, time

def return_user(link, i):
    cookies = {
        '__stripe_mid': '7b95eb48-7e92-492c-bc68-f080d4700cdc527ad7',
        'csrftoken': '9LNO9dIUeEQy2OlTzzdCjWiE7hAbV5l7lZMuJbHu5zjcF8ljvAdVlbrqrd4ZfKPj',
        'c_a_u': 'ZWxhbWlya2Fk:1oNd3S:iLsnvAfAKpYVLmVkpz4aHeWaPd0',
    ...
    response = requests.post('https://leetcode.com/graphql/', cookies=cookies, headers=headers, json=json_data)
    data = json.loads(response.text)
    attended = data['data']['userContestRankingHistory'][-i]['attended']
    trend = data['data']['userContestRankingHistory'][-i]['trendDirection']
    finish = data['data']['userContestRankingHistory'][-i]['finishTimeInSeconds']
    solved = data['data']['userContestRankingHistory'][-i]['problemsSolved']
    rating = round(data['data']['userContestRankingHistory'][-i]['rating'])
    ranking = data['data']['userContestRankingHistory'][-i]['ranking']
    title = data['data']['userContestRankingHistory'][-i]['contest']['title']
    return attended, trend, finish, solved, rating, ranking, title
